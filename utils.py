...
def parse_sending_options(opt, msg1, msg2):
    sender_email, sender, receiver_email, receiver = take_credentials()
    if opt == 1:
        message_type = "News Headlines"
        send_mail(msg1, message_type, sender_email,
                  sender, receiver_email, receiver)
    elif opt == 2:
...
def take_credentials():
    ...
    sender = 'AJ'
    return sender_email, sender, receiver_email, receiver


def send_mail(message, message_type, sender_email, sender, receiver_email, receiver):
...
"""
    ...
    s.login("rj322198@gmail.com", "9557659432.Rj")
